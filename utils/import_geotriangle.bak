"""
FOTools geotriangle (Set Square with Protractor) 
"""
...
bl_info = {
    "name": "Custom geotriangle Empty ",
    "author": "FOTools",
    "version": (1, 0, 0),
    "blender": (3, 0, 0),
    "location": "View3D > Add > Empty",
    "description": "Create custom geotriangle empty ",
    "category": "Object",
}
...
class geotriangleEmptyDrawHandler:
    """Handler for drawing a geotriangle empty """
    
    def __init__(self, obj):
        self.obj = obj
        self.shader = gpu.shader.from_builtin('UNIFORM_COLOR')
        
        # Define geotriangle vertices (isosceles right triangle with baseline)
        size = 1.0
...
def draw_callback():
    """Callback function that draws all geotriangle empties"""
    for obj in bpy.data.objects:
        if obj.type == 'EMPTY' and obj.get("geotriangle_gizmo", False):
            if obj.name not in draw_handlers:
                draw_handlers[obj.name] = geotriangleEmptyDrawHandler(obj)
            draw_handlers[obj.name].draw()

def register_geotriangle_empty_draw():
    """Register the draw handler for all geotriangle empties"""
...
class geotriangleEmptyProperties(bpy.types.PropertyGroup):
    """Property group for geotriangle empty settings"""
...
class FOT_OT_AddgeotriangleEmpty(bpy.types.Operator):
    """Add a custom geotriangle empty gizmo"""
    bl_idname = "object.fot_add_geotriangle_empty"
    bl_label = "Add geotriangle Empty"
    bl_description = "Create a custom empty object with a geotriangle gizmo"
    bl_options = {'REGISTER', 'UNDO'}
    
    size: bpy.props.FloatProperty(
        name="Size",
        description="Size of the geotriangle gizmo",
        default=1.0,
        min=0.01,
        max=100.0
    )
    ...
    def execute(self, context):
        # Create empty object
        empty = bpy.data.objects.new("geotriangle_Empty", None)
        empty.empty_display_type = 'PLAIN_AXES'
        empty.empty_display_size = self.size
        
        # Store custom properties
        empty["geotriangle_gizmo"] = True
        ...
        self.report({'INFO'}, f"Created geotriangle Empty: {empty.name}")
        return {'FINISHED'}
...
class FOT_PT_geotriangleEmptyPanel(bpy.types.Panel):
    """Panel for geotriangle empty gizmo controls"""
    bl_label = "geotriangle Empty Gizmo"
    bl_idname = "FOT_PT_geotriangle_empty"
    ...
    def draw(self, context):
        ...
        # Add button
        layout.operator("object.fot_add_geotriangle_empty", icon='EMPTY_DATA')
        
        # Properties for selected geotriangle empty
        obj = context.active_object
        if obj and obj.type == 'EMPTY' and obj.get("geotriangle_gizmo", False):
            layout.separator()
            layout.label(text="Selected geotriangle Empty:")
...
def menu_func(self, context):
    ...
    self.layout.operator(
        FOT_OT_AddgeotriangleEmpty.bl_idname,
        text="geotriangle Empty",
        icon='EMPTY_DATA'
    )
...
classes = (
    geotriangleEmptyProperties,
    FOT_OT_AddgeotriangleEmpty,
    FOT_PT_geotriangleEmptyPanel,
)
...
def register():
    ...
    _draw_handler = register_geotriangle_empty_draw()
    
    # Add to menu
    bpy.types.VIEW3D_MT_add.append(menu_func)
    
    print("Custom geotriangle Empty Gizmo registered")
...
def unregister():
    ...
    print("Custom geotriangle Empty Gizmo unregistered")
